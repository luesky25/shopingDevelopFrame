...
public class CropImage extends MonitoredActivity {
	...
	public static final String ACTION_CROP_IMAGE = "com.kugou.android.action.CROPIMAGE";
	...
	public void onCreate(Bundle icicle) {
		...
		if (extras != null) {
			...
			mSavePath = extras.getString("savePath");
		...
		if (mBitmap == null) {
			...
			if (mImage != null) {
				...
				if (mBitmap == null) {
					mBitmap = BitmapUtil.readBitmap(mImage.getDataPath());
				}
				File file=new File(intent.getData().getPath());
				if(file.exists()){
					degree = getBitmapDegree(mImage.getDataPath());
				}else{
					initDegreeUri(intent.getData());
				}
			}
		}
		if (mBitmap == null) {
			try {
				mBitmap = MediaStore.Images.Media.getBitmap(mContentResolver,
						intent.getData());
				File file=new File(intent.getData().getPath());
				if(!file.exists()){
					initDegreeUri(intent.getData());
	 ...
	 * @param originalUri
	 */
	private void initDegreeUri(Uri originalUri) {
	...
	private void onRotateRight(int degrees) {
		if (degrees != 0 && mBitmap != null) {
			Matrix m = new Matrix();
			m.setRotate(degrees, (float) mBitmap.getWidth() / 2,
					(float) mBitmap.getHeight() / 2);
			try {
				Bitmap b = BitmapUtil.createBitmap(mBitmap, 0, 0,
		...
		Util.startBackgroundJob(this, null,
				...
				new Runnable() {
					...
					public void run() {
						...
						int width = 0;
						int height = 0;
						Bitmap bmp = (mImage != null) ? mImage.fullSizeBitmap(IImage.UNCONSTRAINED, 1024 * 1024) : mBitmap;
						...
						if (width > 4096 || height > 4096) {
							b = Bitmap.createScaledBitmap(bmp,
									Math.min(width, 4096),
									Math.min(height, 4096), true);
							bmp.recycle();
						} else {
							b = bmp;
						}
	...
	private void onSaveClicked() {
		...
		if (mOutputX != 0 && mOutputY != 0 && !mScale) {
			...
			// required dimension
			croppedImage = BitmapUtil.createBitmap(mOutputX, mOutputY,
			...
			// third param here.
			croppedImage = BitmapUtil.createBitmap(width, height,
			...
			if (mSavePath == null) {
				...
				try {
					...
					if (newUri != null) {
						...
						extras.putParcelable("data", croppedImage);
						setResult(RESULT_OK,
								(new Intent()).setAction("inline-data")
										.putExtras(extras));
					}
				} catch (Exception ex) {
	...
	Runnable mRunFaceDetection = new Runnable() {
		...
		private Bitmap prepareBitmap() {
			...
			Bitmap faceBitmap = BitmapUtil.createBitmap(mBitmap, 0, 0,
