...
//    public static final int STATE_READY = 2;
    ...
    public enum STATE{NORMAL,LOADING,READY};

    public interface StateListener {
        boolean interceptStateChange(RefreshView refreshView, STATE state, STATE oldState);

        void onStateChange(RefreshView refreshView, STATE state);
    }
    ...
    public void setState(STATE state) {
        ...
        if (overScrollManager != null) {
            int lockTop = (state == STATE.LOADING || state == STATE.READY ? getMeasuredHeight() : 0);
    ...
    protected abstract void onStateChange(STATE newState, STATE oldState);
